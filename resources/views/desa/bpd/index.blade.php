<x-app-layout>
    ...
    <div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse ($bpd as $row)
                <div x-data="{}" x-on:click="$dispatch('open-modal', 'detail-{{ $row->id }}')" class="bg-white rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] p-6 flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-lg border border-slate-100 group cursor-pointer">
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <p class="font-medium text-gray-900">{{ $row->desa->nama_desa ?? auth()->user()->desa->nama_desa ?? 'Desa' }} / {{ $row->desa->kecamatan->nama_kecamatan ?? auth()->user()->desa->kecamatan->nama_kecamatan ?? 'Kecamatan' }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">Tanggal Mulai Jabatan</p>
                                <p class="font-medium text-gray-900">{{ $row->tgl_mulai_jabatan ? $row->tgl_mulai_jabatan->format('d/m/y') : '-' }}</p>
                            </div>
                            <div>
